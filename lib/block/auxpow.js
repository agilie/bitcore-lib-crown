...
let scriptSymbol = Buffer.from("fabe6d6d","hex");
...
class AuxPow extends MerkleTransaction {
  ...
  check(blockHash, chainId) {
    ...
    let symbolPos = scriptSig.indexOf(scriptSymbol);
    ...
    if (hashPos === -1) throw new Error("AuxPOW missing chain merkle root in parent coinbase scriptSig.");
